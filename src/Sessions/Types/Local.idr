...
mutual
  ...
  ||| We would like this to be total, however, calling out to generic total functions is seen as non-total
  |||
  ||| We trust this code so let us assume it its total for now.
  |||
  ||| An unrolled form is at the end of this file.
  decEqs : (x,y : List $ Branch Local rs fs)
               -> Dec (x === y)
  decEqs
    = Branches.decEq (assert_total Local.decEq)
  ...
  decEq (Comm cx rx xs) (Comm cy ry ys) with (decEq cx cy)
    decEq (Comm cx rx xs) (Comm cx ry ys) | (Yes Refl) with (decEq rx ry)
      decEq (Comm cx rx xs) (Comm cx rx ys) | (Yes Refl) | (Yes Refl) with (decEqs xs ys)
...
{- [ NOTE ] old code that enables decEq to be total.
mutual
  namespace Branch
    export
    decEq : (x,y : Branch Local rs fs) -> Dec (x === y)
  ...
  namespace Branches
    ...
    export
    decEq : (x,y : List $ Branch Local rs fs) -> Dec (x === y)
    decEq [] [] = Yes Refl
    decEq [] (x :: xs)
      = No rightHeavy
    decEq (x :: xs) []
